...
function check_current_panzoom(buch_id){
    // Panzoom wird NICHT hier initialisiert – das würde pointerdown-Events abfangen
    // und das Ziehen im PageFlip-Buch blockieren. Lazy-Init nur in zoom_in/zoom_out.
    const panzoom = PageFlipRegistry[buch_id]?.panzoom;
    const current = panzoom ? panzoom.getScale() : default_panzoom_zoom;
    let $controll_leiste;
    ...
    show_debug_msg(current + " + " + default_panzoom_zoom);
    if (current == default_panzoom_zoom){
      if ($controll_leiste) $controll_leiste.find(".bt-icon-zoom-standard").addClass("pdf_control_none");
      return true;
    }
    else {
      if ($controll_leiste) $controll_leiste.find(".bt-icon-zoom-standard").removeClass("pdf_control_none");
      return false;
    }
}
...
`.trim();
  ...
  if ($img_files.length === 0 || !$img_files.val()) {
    show_debug_msg("Keine Bilddatei-Metadaten gefunden fuer " + buch_id);
    return;
  }
  ...
  function initializeControls(pageFlip_Instanz) {
    ...
    // Zoom-Reset-Button beim Init verstecken (Zoom startet immer auf Standard).
    // Panzoom wird hier bewusst NICHT initialisiert: eine frühe Init auf .stf__wrapper
...
jQuery(function($) {
  ...
  $('.ui-flipbook').on('dblclick', function (e) {
    ...
    if ($flipbook.attr('data-zoom-dblclick') !== 'true') {
      return;
    }
    ...
    document.querySelectorAll('.ui-flipbook').forEach(function (element) {
...
}); // end main jQuery(function($) {...}) block

})(jQuery); // end noConflict IIFE
